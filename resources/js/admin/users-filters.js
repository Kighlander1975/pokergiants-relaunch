document.addEventListener('DOMContentLoaded', function () {
    ...
    const shouldShowInactiveReset = (roles, statuses) => roles.size > 0 || statuses.size > 0;
    ...
    const updateRadioGroupState = (group) => {
        const groupRadios = document.querySelectorAll(`input[type="radio"][data-status-group="${group}"]`);
    ...
    const setupStatusRadios = () => {
        ...
        const groups = new Set(Array.from(statusRadios).map((radio) => radio.dataset.statusGroup));
