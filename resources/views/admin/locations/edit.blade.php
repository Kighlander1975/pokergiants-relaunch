...
@section('title', 'Spielstätte bearbeiten')
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 space-y-6">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">Spielstätte bearbeiten</h3>
                ...
                <form method="POST" action="{{ route('admin.locations.update', $location) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input required type="text" name="name" id="name" value="{{ old('name', $location->name) }}" class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                        @error('name') <p class="text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="street" class="block text-sm font-medium text-gray-700">Straße / Hausnummer</label>
                        <input required type="text" name="street" id="street" value="{{ old('street', $location->street) }}" class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <input required type="text" name="postal_city" id="postal_city" value="{{ old('postal_city', $location->postal_city) }}" class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <textarea required name="notes" id="notes" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('notes', $location->notes) }}</textarea>
